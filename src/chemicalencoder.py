...
from tensorflow.keras.layers import Input, Dense,GaussianNoise,Normalization
...
class ChemicalEncoder:
    ...
    def create_model(self,layers,activation="relu",dropout=True,optimizer="adam",
                 batch_size=32,loss_func="mse",repeat=False,noise=0.0,scaler="MinMax"):
        ...
        if scaler=="Normalization":
            ...
            self.scaler=lambda x: (x+np.abs(self.min_val))/np.abs(self.min_val)
    ...
    def prepare_inputs(self,df):
        ...
        df=pd.DataFrame(columns=self.encoded_species,data=self.scaler(df))
